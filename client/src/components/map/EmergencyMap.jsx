...
export default function EmergencyMap({ 
  ...
  useEffect(() => {
    ...
    hospitals.forEach(h => {
      ...
      const marker = L.marker([hLat, hLon], { icon: isMatched ? matchedHospitalIcon : hospitalIcon })
        .addTo(map)
        .bindPopup(`
          <div style="font-family: sans-serif; font-size: 12px;">
            ...
            🕒 <b>Hours:</b> ${h.is_24_7 ? '24/7 Open' : `${h.opening_time || '08:00'} - ${h.closing_time || '20:00'}`}<br/>
            📞 ${h.phone || ''}
          </div>
        `);
      ...
      // 3. Draw Route Polyline from Victim to Matched Hospital
      if (isMatched) {
    ...
    // 4. Plot Matched Ambulance Marker if present
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '340px', borderRadius: '12px', overflow: 'hidden', border: '2px solid #CBD5E1' }}>
      ...
      <div style={{
        ...
        🗺 Live GIS GIS Route Engine (Pune District)
      </div>
    </div>
  );
}
